  ...
  Widget _buildBody(BuildContext context, Map<String, dynamic> course) {
    ...
    return CoursePageBody(
      children: [
        ...
        CourseHeaderRow(
          title: courseTr(context, 'Content'),
          trailing: FilledButton.tonal(
            onPressed: () => _showContentDialog(),
            child: Text(courseTr(context, 'Add content')),
          ),
        ),
        const SizedBox(height: 16),
        if (items.isEmpty)
          CourseSectionCard(
            ...
            child: Text(
              courseTr(
                context,
                'No content items yet. Add the first item to make this course usable in the mobile app.',
              ),
            ),
          )
        else
          CourseSectionCard(
            title: courseTr(context, 'All content'),
            child: Column(
              children: [
                for (var index = 0; index < items.length; index++) ...[
                  _OwnerCourseContentRecord(
                    item: courseAsMap(items[index]),
                    onOpen: () => _openContentItem(courseAsMap(items[index])),
                    onEdit: () =>
                        _showContentDialog(item: courseAsMap(items[index])),
                    onDelete: () => _deleteContent(courseAsMap(items[index])),
                  ),
...
class _OwnerCourseContentRecord extends StatelessWidget {
  const _OwnerCourseContentRecord({
    required this.item,
  ...
  @override
  Widget build(BuildContext context) {
    final title = courseReadString(item, 'title').trim().isEmpty
        ? courseTr(context, 'Untitled content')
        : courseReadString(item, 'title');
    final body = courseReadString(item, 'body');
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              CourseStatusChip(
                label: courseReadString(item, 'content_type'),
              ),
              CourseStatusChip(
                label:
                    '${courseTr(context, 'Order')} ${courseReadInt(item, 'order')}',
              ),
            ],
          ),
          ...
          if (materialUrl.trim().isNotEmpty) ...[
            const SizedBox(height: 8),
            Text(
              materialUrl,
              maxLines: 1,
          ...
          Wrap(
            ...
            children: [
              FilledButton.tonal(
                onPressed: onOpen,
                child: Text(courseTr(context, 'Open')),
              ),
              FilledButton.tonal(
                onPressed: onEdit,
                child: Text(courseTr(context, 'Edit')),
              ),
              TextButton(
                onPressed: onDelete,
                child: Text(courseTr(context, 'Delete')),
              ),
            ],
          ),
        ],
